...
server {
    ...
    location / {
        ...
        sub_filter '</body>' '<script>(function(){var pre=document.querySelector("pre");if(!pre){return;}var month={Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11};var observer;function extractTimestamp(line){var text=line.replace(/<[^>]+>/g," ").replace(/&nbsp;/g," ");var m=text.match(/(\\d{2})-([A-Za-z]{3})-(\\d{4})\\s+(\\d{2}):(\\d{2})/);if(m){return Date.UTC(parseInt(m[3],10),month[m[2]],parseInt(m[1],10),parseInt(m[4],10),parseInt(m[5],10));}var nameMatch=line.match(/<a href="[^"]+">([^<]+)<\\/a>/);if(nameMatch){var name=nameMatch[1];var nm=name.match(/(\\d{8})-(\\d{6})/);if(nm){var y=nm[1].slice(0,4),mo=nm[1].slice(4,6),d=nm[1].slice(6,8),h=nm[2].slice(0,2),mi=nm[2].slice(2,4),s=nm[2].slice(4,6);return Date.UTC(parseInt(y,10),parseInt(mo,10)-1,parseInt(d,10),parseInt(h,10),parseInt(mi,10),parseInt(s,10));}}return -Infinity;}function sortList(){if(observer){observer.disconnect();}var lines=pre.innerHTML.trim().split(/\\n+/);if(!lines.length){if(observer){observer.observe(pre,{childList:true,subtree:true});}return;}var parent=lines[0];var items=lines.slice(1).map(function(line){return{raw:line,sort:extractTimestamp(line)};}).sort(function(a,b){return(b.sort||0)-(a.sort||0);});pre.innerHTML=[parent].concat(items.map(function(i){return i.raw;})).join("\\n");if(observer){observer.observe(pre,{childList:true,subtree:true});}}observer=new MutationObserver(sortList);observer.observe(pre,{childList:true,subtree:true});sortList();})();</script></body>';
        sub_filter_once off;
