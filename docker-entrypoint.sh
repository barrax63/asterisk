...
if [ -f "${PJSIP_STASH_PATH}" ]; then
    ...
    if [ ! -f "${PJSIP_PATH}" ]; then
        ...
        REFRESH_PJSIP=true
    elif grep -Eq "sip:(fritz\\.box|[a-zA-Z0-9._-]+@fritz\\.box)(:[0-9]+)?" "${PJSIP_PATH}"; then
    ...
    if [ "${REFRESH_PJSIP}" = true ]; then
        if ! cp -f "${PJSIP_STASH_PATH}" "${PJSIP_PATH}"; then
            echo "WARNING: Failed to refresh pjsip.conf from template at ${PJSIP_STASH_PATH}. Check file permissions and available disk space."
        fi
    fi
fi
